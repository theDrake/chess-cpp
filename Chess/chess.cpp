...
//				chess moves. Does not actually simulate a chess game or allow
//				the user to play chess. (Note: Y is the "up" direction!)
//*****************************************************************************
...
void Interpolate(double t1, double t, double t2, double x1, double &x, double x2)
{
	if (t < t1)
	{
		x = x1;
...
void display(void)
{
	...
	{
		Interpolate(0, currentTime, INTRO_ZOOM_DURATION, INITIAL_EYE_X, eye[0], DEFAULT_EYE_X);
	}
	else
	{
	...
	for (int i = 0; i < 8; ++i)
	{
		...
		{
			glBegin(GL_QUADS);
			glVertex3d(BOARD_SQUARE_ORIGIN_X + i * BOARD_SQUARE_SIZE + BOARD_SQUARE_BORDER,
					   BOARD_TOP + Y_MODIFIER * 2,
					   BOARD_SQUARE_ORIGIN_Z + j * BOARD_SQUARE_SIZE + BOARD_SQUARE_BORDER);
			glVertex3d(BOARD_SQUARE_ORIGIN_X + i * BOARD_SQUARE_SIZE + BOARD_SQUARE_BORDER,
					   BOARD_TOP + Y_MODIFIER * 2,
					   BOARD_SQUARE_ORIGIN_Z + j * BOARD_SQUARE_SIZE + BOARD_SQUARE_SIZE - BOARD_SQUARE_BORDER);
			glVertex3d(BOARD_SQUARE_ORIGIN_X + i * BOARD_SQUARE_SIZE + BOARD_SQUARE_SIZE - BOARD_SQUARE_BORDER,
					   BOARD_TOP + Y_MODIFIER * 2,
					   BOARD_SQUARE_ORIGIN_Z + j * BOARD_SQUARE_SIZE + BOARD_SQUARE_SIZE - BOARD_SQUARE_BORDER);
			glVertex3d(BOARD_SQUARE_ORIGIN_X + i * BOARD_SQUARE_SIZE + BOARD_SQUARE_SIZE - BOARD_SQUARE_BORDER,
					   BOARD_TOP + Y_MODIFIER * 2,
	...
	for(int i = 1; i <= 8; ++i)
	{
		...
		if (i == 1)
		{
			if (currentTime <= INTRO_ZOOM_DURATION + 2)
				Interpolate(INTRO_ZOOM_DURATION + 1, currentTime, INTRO_ZOOM_DURATION + 2, 2000, z, 4000);
			else
				Interpolate(INTRO_ZOOM_DURATION + 3, currentTime, INTRO_ZOOM_DURATION + 4, 4000, z, 5000);
			Interpolate(INTRO_ZOOM_DURATION + 3, currentTime, INTRO_ZOOM_DURATION + 4, i * 1000, x, i * 1000 + 1000);
		}
		glPushMatrix();
		glTranslatef(x, 0, z);
		glCallList(PAWN);
	...
	// Knight 2
	Interpolate(INTRO_ZOOM_DURATION + 4.5, currentTime, INTRO_ZOOM_DURATION + 5, 7000, x, 6000);
	Interpolate(INTRO_ZOOM_DURATION + 4, currentTime, INTRO_ZOOM_DURATION + 5, 8000, z, 6000);
	glPushMatrix();
	glTranslatef(x, 0, z);
	glRotatef(180, 0, 1, 0);
	glCallList(KNIGHT);
	...
	// Rook 1
	Interpolate(INTRO_ZOOM_DURATION + 6, currentTime, INTRO_ZOOM_DURATION + 7, 8000, z, 7000);
	glPushMatrix();
	...
	for(int i = 1; i <= 8; ++i)
	{
		...
		if (i == 1)
		{
			Interpolate(INTRO_ZOOM_DURATION + 5.9, currentTime, INTRO_ZOOM_DURATION + 6.4, 0, angle, 180);
			if (currentTime > INTRO_ZOOM_DURATION + 6.4)
				y = -100;
		}
		else if (i == 2)
		{
			Interpolate(INTRO_ZOOM_DURATION + 2, currentTime, INTRO_ZOOM_DURATION + 3, 7000, z, 5000);
			Interpolate(INTRO_ZOOM_DURATION + 3.75, currentTime, INTRO_ZOOM_DURATION + 4.25, 0, angle, 180);
...
void InitializeMyStuff()
{
	...
	GLfloat light_position[] = { -eye[0], -eye[1], -eye[2], 0 }; // Last arg: 0 = vector light (no origin point), 1 = point light.
...
int main(int argc, char **argv)
{
	...
	cout << "CONTROLS:" << endl;
	cout << " 1. Use the mouse buttons, arrow keys, or 'WSAD' keys to adjust the camera position." << endl;
